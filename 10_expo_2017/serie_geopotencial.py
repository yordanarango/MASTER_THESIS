...
# def ciclo_diurno_anual_vector(Serie, fechas):
...
criterio = u'mean_75_100_dur_75_100_PN_y_evento_precedente_en_PP_pero_no_en_TT'#u'mean_75_100_dur_75_100_PN_y_eventos_precedentes_en_PP_y_TT' #u'mean_75_100_dur_75_100_TT_y_posteriores_eventos_en_PP_y_PN'
fechas_cumplen = []
...
if criterio == u'mean_75_100_dur_75_100_PN_y_eventos_precedentes_en_PP_y_TT':
...
if criterio == u'mean_75_100_dur_75_100_PN_y_evento_precedente_en_PP_pero_no_en_TT':
	...
	for date_PN in Dmean_75_100_lon_75_100_PN_max:
		...
		for date_PP in Dates_PP_max:
			if np.any(pd.DatetimeIndex(fecha_PP) == date_PP) == True:
				if list(set(Dates_TT_max).intersection(fecha_TT)) == []:
...
Height = [1000]
for H in Height: 
	os.mkdir(u'/home/yordan/YORDAN/UNAL/TESIS_MAESTRIA/10_expo_2017/SERIES_GEO_POT/'+str(H)+'hPa/'+criterio, 0777)
	os.mkdir(u'/home/yordan/YORDAN/UNAL/TESIS_MAESTRIA/10_expo_2017/SERIES_GEO_POT/'+str(H)+'hPa/'+criterio+'/Escala_por_serie', 0777)
	"Datos geopotencial"
	...
	NA    = G['z'][:, pos_NA_lat, pos_NA_lon]      ;  anom_NA   = anomalias(NA,   dates_ERA)
	Go    = G['z'][:, pos_G_lat, pos_G_lon]        ;  anom_Go   = anomalias(Go,   dates_ERA)
	TT    = G['z'][:, pos_TT_lat, pos_TT_lon]      ;  anom_TT   = anomalias(TT,   dates_ERA)
	PP_C  = G['z'][:, pos_PP_C_lat, pos_PP_C_lon]  ;  anom_PP_C = anomalias(PP_C, dates_ERA)
	PP    = G['z'][:, pos_PP_lat, pos_PP_lon]      ;  anom_PP   = anomalias(PP,   dates_ERA)
	PN_C  = G['z'][:, pos_PN_C_lat, pos_PN_C_lon]  ;  anom_PN_C = anomalias(PN_C, dates_ERA)
	PN    = G['z'][:, pos_PN_lat, pos_PN_C_lon]    ;  anom_PN   = anomalias(PN,   dates_ERA)
	...
	for D in fechas_cumplen: # Se pinta una sóla escala para todas las series
		...
		plt.grid(True)
		plt.legend(loc='best')
		plt.savefig(u'/home/yordan/YORDAN/UNAL/TESIS_MAESTRIA/10_expo_2017/SERIES_GEO_POT/'+str(H)+'hPa/'+criterio+'/'+str(D)+'.png', bbox_inches='tight', dpi=300)
	...
	for D in fechas_cumplen: # Se pintan en una escala particular para cada serie
		...
		plt.savefig(u'/home/yordan/YORDAN/UNAL/TESIS_MAESTRIA/10_expo_2017/SERIES_GEO_POT/'+str(H)+'hPa/'+criterio+'/Escala_por_serie/'+str(D)+'.png', bbox_inches='tight', dpi=300)
		#plt.show()
