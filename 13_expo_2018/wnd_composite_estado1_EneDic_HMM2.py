...
"Se extraen fechas en estado 1, para un modelo con dos estados"
normal_years = np.array([x for x in set(DATES[~DATES.is_leap_year].year)]) #Años normales. NO bisiestos

DT_st1          = [] 
year_cualquiera = pd.date_range('2001-01-01', '2001-12-31', freq='D')   #año cualquiera (bisiesto) para poder extraer mes y día
for i, d in enumerate(normal_years):
	for j in range(365):
		if state_matrix_2st[i, j] == 1:
			DT_st1.append(str(d)+'-'+str(year_cualquiera[j].month)+'-'+str(year_cualquiera[j].day)+' -18:00:00')

DT = pd.DatetimeIndex(DT_st1) #Vuelvo fechas de pandas


"Se seleccionan fechas entre Noviembre 1 a Diciembre 31 y de Enero 1 a Abril 15, en estado 1"
"Se seleccionan fechas entre Abril 16 y Junio 30, y Agosto 16 y Octubre 31, y en estado 1"

dt_EneAbr = [] #fechas de Enero 1 a Abril 15 en estado uno
dt_NovDic = [] #fechas de Noviembre 1 a Diciembre 31 en estado uno

dt_AbrJun = [] #fechas de abril 16 a junio 30 en estado uno
dt_AugOct = [] #fechas de agosto 16 a octubre 31 en estado uno

for d in normal_years:

	pos_EA = np.where((str(d)+'-01-01' <= DT) & (DT <= str(d)+'-04-15'))[0] # posiciones de fechas de Enero 1 a Abril 15 en estado uno 
	pos_ND = np.where((str(d)+'-11-01' <= DT) & (DT <= str(d)+'-12-31'))[0] # posiciones de fechas de Noviembre 1 a Diciembre 31 en estado uno

	pos_AJ = np.where((str(d)+'-04-16' <= DT) & (DT <= str(d)+'-06-30'))[0] # posiciones de fechas de abril 16 a junio 30 en estado uno 
	pos_AO = np.where((str(d)+'-08-16' <= DT) & (DT <= str(d)+'-10-31'))[0] # posiciones de fechas de agosto 16 a octubre 31 en estado uno
...
"Se crea la matríz con datos de Noviembre hasta Abril"
#CompU_NovAbr = np.concatenate((CompU_EneAbr, CompU_NovDic))
#CompV_NovAbr = np.concatenate((CompV_EneAbr, CompV_NovDic))

"Se calcula la velocidad de las anomalías del viento para los estados 1 entre Ene-Abr, Nov-Dic y Nov-Abr. Se plotea"
COMPU_st1_EneAbr = np.mean(CompU_EneAbr, axis = 0); COMPV_st1_EneAbr = np.mean(CompV_EneAbr, axis = 0); COMPWnd_st1_EneAbr = np.sqrt(COMPU_st1_EneAbr*COMPU_st1_EneAbr + COMPV_st1_EneAbr*COMPV_st1_EneAbr); min_EA = np.min(COMPWnd_st1_EneAbr); max_EA = np.max(COMPWnd_st1_EneAbr); Ttl_EA = 'January-April Wind Composites - State 1 (HMM 2)'; path_EA = '/home/yordan/YORDAN/UNAL/TESIS_MAESTRIA/13_expo_2018/CompWind_EA_1st'
COMPU_st1_NovDic = np.mean(CompU_NovDic, axis = 0); COMPV_st1_NovDic = np.mean(CompV_NovDic, axis = 0); COMPWnd_st1_NovDic = np.sqrt(COMPU_st1_NovDic*COMPU_st1_NovDic + COMPV_st1_NovDic*COMPV_st1_NovDic); min_ND = np.min(COMPWnd_st1_NovDic); max_ND = np.max(COMPWnd_st1_NovDic); Ttl_ND = 'November-December Wind Composites - State 1 (HMM 2)'; path_ND = '/home/yordan/YORDAN/UNAL/TESIS_MAESTRIA/13_expo_2018/CompWind_ND_1st'
#COMPU_st1_NovAbr = np.mean(CompU_NovAbr, axis = 0); COMPV_st1_NovAbr = np.mean(CompV_NovAbr, axis = 0); COMPWnd_st1_NovAbr = np.sqrt(COMPU_st1_NovAbr*COMPU_st1_NovAbr + COMPV_st1_NovAbr*COMPV_st1_NovAbr); min_NA = np.min(COMPWnd_st1_NovAbr); max_NA = np.max(COMPWnd_st1_NovAbr); Ttl_NA = 'November-April Wind Composites - State 1 (HMM 2)'; path_NA = '/home/yordan/YORDAN/UNAL/TESIS_MAESTRIA/13_expo_2018/CompWind_NA_1st'
COMPU_st1_AbrJun = np.mean(CompU_AbrJun, axis = 0); COMPV_st1_AbrJun = np.mean(CompV_AbrJun, axis = 0); COMPWnd_st1_AbrJun = np.sqrt(COMPU_st1_AbrJun*COMPU_st1_AbrJun + COMPV_st1_AbrJun*COMPV_st1_AbrJun); min_AJ = np.min(COMPWnd_st1_AbrJun); max_AJ = np.max(COMPWnd_st1_AbrJun); Ttl_AJ = 'April-Junio Wind Composites - State 1 (HMM 2)'   ; path_AJ = '/home/yordan/YORDAN/UNAL/TESIS_MAESTRIA/13_expo_2018/CompWind_AJ_1st'
COMPU_st1_AugOct = np.mean(CompU_AugOct, axis = 0); COMPV_st1_AugOct = np.mean(CompV_AugOct, axis = 0); COMPWnd_st1_AugOct = np.sqrt(COMPU_st1_AugOct*COMPU_st1_AugOct + COMPV_st1_AugOct*COMPV_st1_AugOct); min_AO = np.min(COMPWnd_st1_AugOct); max_AO = np.max(COMPWnd_st1_AugOct); Ttl_AO = 'August-October Wind Composites - State 1 (HMM 2)'; path_AO = '/home/yordan/YORDAN/UNAL/TESIS_MAESTRIA/13_expo_2018/CompWind_AO_1st'


MIN = np.min([min_EA, min_ND, min_AJ, min_AO]) # min_NA
MAX = np.max([max_EA, max_ND, max_AJ, max_AO]) # max_NA
    
plotear(lat[-1], lat[0], lon[0], lon[-1], 4, 7, lon[::2], lat[::2], COMPWnd_st1_EneAbr[::2, ::2], MIN, MAX, 'm/s', Ttl_EA, path_EA, C_T='k', wind=True, mapa_u=COMPU_st1_EneAbr[::2, ::2], mapa_v=COMPV_st1_EneAbr[::2, ::2])
plotear(lat[-1], lat[0], lon[0], lon[-1], 4, 7, lon[::2], lat[::2], COMPWnd_st1_NovDic[::2, ::2], MIN, MAX, 'm/s', Ttl_ND, path_ND, C_T='k', wind=True, mapa_u=COMPU_st1_NovDic[::2, ::2], mapa_v=COMPV_st1_NovDic[::2, ::2]) 
#plotear(lat[-1], lat[0], lon[0], lon[-1], 4, 7, lon[::2], lat[::2], COMPWnd_st1_NovAbr[::2, ::2], MIN, MAX, 'm/s', Ttl_NA, path_NA, C_T='k', wind=True, mapa_u=COMPU_st1_NovAbr[::2, ::2], mapa_v=COMPV_st1_NovAbr[::2, ::2]) 
plotear(lat[-1], lat[0], lon[0], lon[-1], 4, 7, lon[::2], lat[::2], COMPWnd_st1_AbrJun[::2, ::2], MIN, MAX, 'm/s', Ttl_AJ, path_AJ, C_T='k', wind=True, mapa_u=COMPU_st1_AbrJun[::2, ::2], mapa_v=COMPV_st1_AbrJun[::2, ::2])
plotear(lat[-1], lat[0], lon[0], lon[-1], 4, 7, lon[::2], lat[::2], COMPWnd_st1_AugOct[::2, ::2], MIN, MAX, 'm/s', Ttl_AO, path_AO, C_T='k', wind=True, mapa_u=COMPU_st1_AugOct[::2, ::2], mapa_v=COMPV_st1_AugOct[::2, ::2]) 
