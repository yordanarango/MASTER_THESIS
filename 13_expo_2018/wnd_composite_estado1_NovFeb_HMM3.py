...
"Se extraen fechas en estado 1, para un modelo con tres estados"
...
for i, d in enumerate(normal_years[1:]-1):
	for j in range(state_matrix_3st.shape[1]):
		if state_matrix_3st[i, j] == 1:
			MONTH = year_cualquiera[j].month
			if MONTH <= 2:
				DT_st1.append(str(d+1)+'-'+str(year_cualquiera[j].month)+'-'+str(year_cualquiera[j].day)+' -18:00:00')
			if MONTH >= 11:
...
"Se hacen compuestos para Diciembre-Febrero"
