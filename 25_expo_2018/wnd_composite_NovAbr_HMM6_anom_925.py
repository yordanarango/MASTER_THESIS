...
def plotear(lllat, urlat, lllon, urlon, dist_lat, dist_lon, Lon, Lat, mapa, bar_min, bar_max, unds, titulo, path, C_T='k', wind=False, mapa_u=None, mapa_v=None):
    ...
    if wind == True:
        Q = map.quiver(x[::4,::4], y[::4,::4], mapa_u[::4,::4], mapa_v[::4,::4], scale=40)
...
for ch in ['TT', 'PP', 'PN']: #Selección de chorro
    BC = 'todos' # individual o conjunto
    ...
    if ch == 'TT' and NM == 6:
    	states6[states6 == 2] = 33
    	states6[states6 == 3] = 11
    	states6[states6 == 5] = 22
    	states6[states6 == 1] = 55

    	states6[states6 == 11] = 1
    	states6[states6 == 22] = 2
    	states6[states6 == 33] = 3
    	states6[states6 == 55] = 5

    if ch == 'PP' and NM == 6:
    	states6[states6 == 2] = 44
    	states6[states6 == 4] = 22

    	states6[states6 == 22] = 2
    	states6[states6 == 44] = 4
    ...
    for k in range(1, NM+1):
        ...
        if NM == 6: # Se arregla el vector de estados
            ST = states6
        ...
        path.append('/home/yordan/YORDAN/UNAL/TESIS_MAESTRIA/25_expo_2018/WND_Composites/' + BC + '/' + ch + '_CompWind_NovAbr_st'+str(S)+'_HMM'+str(NM)+'_'+BC+'_anom_925')
    ...
    if BC == 'conjunto':
        ...
        # Estado 4
        plotear(lat[-1], lat[0], lon[0], lon[-1], 4, 7, lon, lat, Comp_spd[3], np.min(Min_spd), np.max(Max_spd), 'm/s', Ttl[3], path[3], C_T='k', wind=True, mapa_u=Comp_u[2], mapa_v=Comp_v[2])
        # Estado 5
        plotear(lat[-1], lat[0], lon[0], lon[-1], 4, 7, lon, lat, Comp_spd[4], np.min(Min_spd), np.max(Max_spd), 'm/s', Ttl[4], path[4], C_T='k', wind=True, mapa_u=Comp_u[2], mapa_v=Comp_v[2])
        # Estado 6
        plotear(lat[-1], lat[0], lon[0], lon[-1], 4, 7, lon, lat, Comp_spd[5], np.min(Min_spd), np.max(Max_spd), 'm/s', Ttl[5], path[5], C_T='k', wind=True, mapa_u=Comp_u[2], mapa_v=Comp_v[2])
        ...
        # Estado 4
        plotear(lat[-1], lat[0], lon[0], lon[-1], 4, 7, lon, lat, Comp_spd[3], Min_spd[3], Max_spd[3], 'm/s', Ttl[3], path[3], C_T='k', wind=True, mapa_u=Comp_u[2], mapa_v=Comp_v[2])
        # Estado 5
        plotear(lat[-1], lat[0], lon[0], lon[-1], 4, 7, lon, lat, Comp_spd[4], Min_spd[4], Max_spd[4], 'm/s', Ttl[4], path[4], C_T='k', wind=True, mapa_u=Comp_u[2], mapa_v=Comp_v[2])
        # Estado 6
        plotear(lat[-1], lat[0], lon[0], lon[-1], 4, 7, lon, lat, Comp_spd[5], Min_spd[5], Max_spd[5], 'm/s', Ttl[5], path[5], C_T='k', wind=True, mapa_u=Comp_u[2], mapa_v=Comp_v[2])

    elif BC == 'todos':
        # Estado 1
        plotear(lat[-1], lat[0], lon[0], lon[-1], 4, 7, lon, lat, Comp_spd[0], 0, 16.3, 'm/s', Ttl[0], path[0], C_T='k', wind=True, mapa_u=Comp_u[0], mapa_v=Comp_v[0])
        # Estado 2
        plotear(lat[-1], lat[0], lon[0], lon[-1], 4, 7, lon, lat, Comp_spd[1], 0, 16.3, 'm/s', Ttl[1], path[1], C_T='k', wind=True, mapa_u=Comp_u[1], mapa_v=Comp_v[1])
        # Estado 3
        plotear(lat[-1], lat[0], lon[0], lon[-1], 4, 7, lon, lat, Comp_spd[2], 0, 16.3, 'm/s', Ttl[2], path[2], C_T='k', wind=True, mapa_u=Comp_u[2], mapa_v=Comp_v[2])
        # Estado 4
        plotear(lat[-1], lat[0], lon[0], lon[-1], 4, 7, lon, lat, Comp_spd[3], 0, 16.3, 'm/s', Ttl[3], path[3], C_T='k', wind=True, mapa_u=Comp_u[2], mapa_v=Comp_v[2])
        # Estado 5
        plotear(lat[-1], lat[0], lon[0], lon[-1], 4, 7, lon, lat, Comp_spd[4], 0, 16.3, 'm/s', Ttl[4], path[4], C_T='k', wind=True, mapa_u=Comp_u[2], mapa_v=Comp_v[2])
        # Estado 6
        plotear(lat[-1], lat[0], lon[0], lon[-1], 4, 7, lon, lat, Comp_spd[5], 0, 16.3, 'm/s', Ttl[5], path[5], C_T='k', wind=True, mapa_u=Comp_u[2], mapa_v=Comp_v[2])
