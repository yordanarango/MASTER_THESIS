...
"Lectura de Estados"
rf     = open("/home/yordan/YORDAN/UNAL/TESIS_MAESTRIA/21_expo_2018/States_TT_NovMar.csv", 'r')
...
if Nc == 2:
    state_matrix = np.reshape(states2_new, (len(states2_new)/152, 152))
if Nc == 3:
    state_matrix = np.reshape(states3_new, (len(states3_new)/152, 152))
if Nc == 4:
    state_matrix = np.reshape(states4_new, (len(states4_new)/152, 152))
if Nc == 5:
    state_matrix = np.reshape(states5_new, (len(states5_new)/152, 152))
...
if Nc == 3:
    ...
    state_matrix[state_matrix == 4] = 11
...
ax.set_title(str(Nc)+' States - TT Wind (Nov-Mar)', fontsize=18)

plt.savefig('/home/yordan/YORDAN/UNAL/TESIS_MAESTRIA/21_expo_2018/Vit_matrix_TT_NovMar_'+str(Nc)+'st.png', bbox_inches='tight', dpi=300)
