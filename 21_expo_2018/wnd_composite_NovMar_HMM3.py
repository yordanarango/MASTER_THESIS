...
a = open('/media/yordan/YORDAN_USB/ciclo_diurno_anual_wind_025_6h.bin', 'rb')
b = open('/media/yordan/YORDAN_USB/ciclo_diurno_anual_U_025_6h.bin', 'rb')
c = open('/media/yordan/YORDAN_USB/ciclo_diurno_anual_V_025_6h.bin', 'rb')
...
"Selección de fechas en Noviembre, Diciembre, Enero, Febrero, Marzo"
FCH = DATES[3::4]
DT  = []
...
for i in years[:-1]:
    ...
    if np.any(normal_years == i+1) == True:
    	DT.append(FCH[pos:pos+151])
    else:
    	DT.append(FCH[pos:pos+152])

FECHAS = pd.DatetimeIndex(np.concatenate(DT))


"Lectura de Estados"
rf     = open('/media/yordan/YORDAN_USB/States_'+ch+'_NovMar.csv', 'r')
...
for k in range(1, NM+1):
    ...
    path.append('/home/yordan/Escritorio/WND_Composites/' + ch + '_CompWind_NovMar_st'+str(S)+'_HMM'+str(NM)+'_'+BC)
    Ttl.append('Nov-Mar Wind Composite \n' + ch + ' - State ' + str(S) + ' (HMM ' + str(NM) + ')')
...
if BC == 'conjunto':
    ...
    # Estado 1
    plotear(lat[-1], lat[0], lon[0], lon[-1], 4, 7, lon, lat, Comp_spd[0], Min_spd[0], Max_spd[0], 'm/s', Ttl[0], path[0], C_T='k', wind=True, mapa_u=Comp_u[0, ::2, ::2], mapa_v=Comp_v[0, ::2, ::2])
    # Estado 2
    plotear(lat[-1], lat[0], lon[0], lon[-1], 4, 7, lon, lat, Comp_spd[1], Min_spd[1], Max_spd[1], 'm/s', Ttl[1], path[1], C_T='k', wind=True, mapa_u=Comp_u[1, ::2, ::2], mapa_v=Comp_v[1, ::2, ::2])
    # Estado 3
    plotear(lat[-1], lat[0], lon[0], lon[-1], 4, 7, lon, lat, Comp_spd[2], Min_spd[2], Max_spd[2], 'm/s', Ttl[2], path[2], C_T='k', wind=True, mapa_u=Comp_u[2, ::2, ::2], mapa_v=Comp_v[2, ::2, ::2])
