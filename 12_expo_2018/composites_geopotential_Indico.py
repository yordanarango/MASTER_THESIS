...
path_figures = u'/home/yordan/YORDAN/UNAL/TESIS_MAESTRIA/12_expo_2018/COMPOSITES_GEO_POT_INDICO/'
...
for H in Height: 
	...
	lat       = G['latitude'][:]
	lon       = G['longitude'][:]-360
	lat_inf   = np.where(lat == -20)[0][0]
	lat_sup   = np.where(lat == 20)[0][0]
	lon_inf   = np.where(lon == -30)[0][0]
	lon_sup   = np.where(lon == -310)[0][0]
	...
	H_composites = ['-8d', '-7.75d', '-7.5d', '-7.25d', '-7d', '-6.75d', '-6.5d', '-6.25d', '-6d', '-5.75d', '-5.5d', '-5.25d', '-5d', '-4.75', '-4.5d', '-4.25d', '-4d', '-3.75d', '-3.5d', '-3.25d', '-3d', '-2.75d', '-2.5d', '-2.25d', '-2d', '-1.75d', '-1.5d','-1.25d','-1d','-0.75d','-0.5d','-0.25d','0d']
						...
						if cri1 != cri2:
							...
							for st, ST in enumerate(MESES):
								...
								if len(fechas_max) != 0:
									...
									for k, i in enumerate(range(-32,1)): # Todo este for es para meter los datos de cada evento
									...
									Dict_comp.update({ch+'_'+cri1+'_'+perc1+'_'+cri2+'_'+perc2+'_'+ST:ANOM_GEO})
	...
	for num, names in enumerate(Dict_comp.keys()):
		...
		for k, i, h in zip(range(len(H_composites)), range(-32,1), H_composites):
			print names + ' ' + str(k) + ' ' + str(len(Dict_comp.keys())) + ' ' + str(num)
			Anom_geo = ANOM[k]
			Pos    = '%02d' % (k,)
			N_eventos = Dict_Num_fechas[names]
			titulo = u'Compuesto de anomalías de altura geopotencial - '+str(H)+' hPa (Max '+h+')'+ '\n' +names+' ('+str(N_eventos)+' events)'
			path   = path_figures+str(H)+'hPa/'+names[:2]+'/'+names[-3:]+'/'+names[:-4]+'/'+Pos+'_'+names[3:]+'_'+h
