...
def plotear(lllat, urlat, lllon, urlon, dist_lat, dist_lon, Lon, Lat, mapa, bar_min, bar_max, unds, titulo, path, C_T='k', wind=False, mapa_u=None, mapa_v=None):
    ...
    bounds = np.linspace(bar_min, bar_max, 30)
    bounds = np.around(bounds, decimals=2)

    if wind == False:
        CF1 = map.contourf(x,y,mapa, 30, norm=MidpointNormalize(midpoint=0), cmap=plt.cm.gist_earth, levels=bounds, extend='max')#plt.cm.rainbow , plt.cm.RdYlBu_r, plt.cm.viridis
        CF2 = map.contourf(x,y,mapa, 30, norm=MidpointNormalize(midpoint=0), cmap=plt.cm.gist_earth, levels=bounds, extend='min')#plt.cm.rainbow, plt.cm.RdYlBu_r, plt.cm.viridis
    else:
        CF1 = map.contourf(x,y,mapa, 30, cmap= plt.cm.rainbow, levels=bounds, extend='max')#plt.cm.rainbow , plt.cm.RdYlBu_r
        CF2 = map.contourf(x,y,mapa, 30, cmap= plt.cm.rainbow, levels=bounds, extend='min')#plt.cm.rainbow, plt.cm.RdYlBu_r
...
a = open('/home/yordan/Escritorio/ciclo_diurno_anual_presure_025_6h.bin', 'rb')
