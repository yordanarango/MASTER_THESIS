...
for ch in ['TT']: #Selección de chorro
    BC = 'individual' # individual o conjunto o todos
    ...
    for i in years[:-1]:
        ...
        pos = np.where(FCH == pd.Timestamp(str(i)+'-11-01 00:00:00'))[0][0]

        if np.any(normal_years == i+1) == True:
        	DT.append(FCH[pos:pos+181*4])
        else:
        	DT.append(FCH[pos:pos+182*4])

    FECHAS = pd.DatetimeIndex(np.concatenate(DT))


    "Lectura de Estados"
    rf     = open('/home/yordan/YORDAN/UNAL/TESIS_MAESTRIA/27_expo_2018/States_'+ch+'_NovAbr_anom_925.csv', 'r')
    ...
    for k in range(1, NM+1):
        ...
        for i, j in enumerate(pos_nc):
            ...
            cc_U = CICLO_U[MESES[mes-1]+'_'+str(hora)]    # Porque los estados se hicieron para la hora de las 18 horas que es cuando se da la mayor velocidad en el día, y fue con lo que se hicieron los HMM
            cc_V = CICLO_V[MESES[mes-1]+'_'+str(hora)]    # Porque los estados se hicieron para la hora de las 18 horas que es cuando se da la mayor velocidad en el día, y fue con lo que se hicieron los HMM
        ...
        path.append('/home/yordan/YORDAN/UNAL/TESIS_MAESTRIA/27_expo_2018/WND_Composites/' + BC + '/' + ch + '_CompWind_NovAbr_st'+str(S)+'_HMM'+str(NM)+'_'+BC+'_anom_925')
