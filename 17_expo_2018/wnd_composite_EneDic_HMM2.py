...
FECHAS         = DATES[3 : pos_2015_12_31+1 : 4]
...
if ch == 'TT' and NM == 2:
    ...
    states2[states2 == 1] = 22
    states2[states2 == 2] = 11

    states2[states2 == 11] = 1
    states2[states2 == 22] = 2
...
for k in range(1, NM+1):
    ...
    if NM == 2: # Se arregla el vector de estados
        ST = states2
    ...
    Comp_u[k-1] = np.mean(CompU, axis = 0); Comp_v[k-1] = np.mean(CompV, axis = 0); Comp_spd[k-1] = np.sqrt(Comp_u[k-1] * Comp_u[k-1] + Comp_v[k-1] * Comp_v[k-1]); Min_spd.append(np.min(Comp_spd[k-1])); Max_spd.append(np.max(Comp_spd[k-1])); Ttl.append('Jan-Dec Wind Composites \n' + ch + '- State ' + str(S) + ' (HMM ' + str(NM) + ')'); path.append('/home/yordan/YORDAN/UNAL/TESIS_MAESTRIA/17_expo_2018/' + ch + 'CompWind_JanDec_st'+str(S)+'_HMM'+str(NM))
