...
for l, rz in enumerate([0, 12, 24, 36, 48, 60, 72, 96, 120, 144, 168, 192, 216, 240, 288, 336, 480, 600, 720, 840, 960]):   #número de horas de rezago en horas, en multiplos de 6, empezando de cero
    ...
    for k in range(1, NM+1):
        ...
        path.append('/home/yordan/YORDAN/UNAL/TESIS_MAESTRIA/23_expo_2018/SST_COMPOSITES/' + '%.2d' % l + '_' + ch + '_CompSST_NovAbr_st'+str(S)+'_HMM'+str(NM)+'_rezago_'+str(rz)+'h_anom')

        if rz <= 72:
            Ttl.append('Nov-Apr SST Anomalies Composite \n' + ch + ' - State ' + str(S) + ' (HMM ' + str(NM) + ') + ' + str(rz) + 'h')
        else:
