...
#==============================================================================
'Funcion plotear perfil'
#==============================================================================

# def plot_function(x, y, title, path, name, Anomalies=False): 
#     fig = plt.figure(figsize=(5,8), edgecolor='W',facecolor='W')
...
#     ax1.set_xlim(11, 30)
#     ax1.invert_yaxis()
#     ax1.set_ylabel(u'Prof (m)')
#     ax1.set_xlabel(u'Temperatura °C')
#     plt.grid(True)
#     plt.savefig(path+name+'.png',dpi=100,bbox_inches='tight')
...
#str_TT = ['1993-03-14 00:00:00', '1996-02-05 06:00:00', '2003-03-31 00:00:00', '2003-11-29 06:00:00', '2007-03-05 06:00:00', '2008-01-03 00:00:00', '2013-03-03 06:00:00']
# str_PP = ['1993-03-15 12:00:00', '1998-01-01 12:00:00', '1998-03-12 12:00:00']
# str_PN = ['1998-03-12 12:00:00', '2003-04-01 12:00:00', '2006-11-23 00:00:00']

#str_TTc = ['1993-03-12 00:00:00', '1996-01-31 00:00:00', '2003-03-27 00:00:00', '2003-11-27 00:00:00', '2007-02-23 00:00:00', '2007-12-28 00:00:00', '2013-02-22 00:00:00']
#str_TTf = ['1993-03-18 00:00:00', '1996-02-12 00:00:00', '2003-04-10 00:00:00', '2003-12-09 00:00:00', '2007-03-10 00:00:00', '2008-01-12 00:00:00', '2013-03-13 00:00:00']

# str_PPc = ['1993-03-05 00:00:00', '1997-12-20 00:00:00', '1998-03-01 00:00:00', '1998-05-02 00:00:00', '2003-03-21 00:00:00', '2008-04-14 00:00:00', '2012-07-16 00:00:00']
# str_PPf = ['1993-03-30 00:00:00', '1998-01-15 00:00:00', '1998-03-25 00:00:00', '1998-05-30 00:00:00', '2003-04-15 00:00:00', '2008-05-18 00:00:00', '2012-08-12 00:00:00']

# str_PNc = ['1998-03-08 00:00:00', '2003-03-28 00:00:00', '2006-11-17 00:00:00']
# str_PNf = ['1998-03-16 00:00:00', '2003-04-05 00:00:00', '2006-12-27 00:00:00']

#==============================================================================
'Ploteando eventos ST'
#==============================================================================

# depth = ArchivoPN['depth'][:]

# for c, f, e in zip(str_PNc, str_PNf, str_PN):
    
#     date_evento = pd.date_range(c, f, freq='D')
#     C = np.where(fechas == c)[0][0]
#     F = np.where(fechas == f)[0][0]

#     ST      = data_PN[C:F+1]
#     ST_anom = anom_PN[C:F+1]

#     path = '/home/yordan/YORDAN/UNAL/TESIS_MAESTRIA/4_expo_2017/perfiles/PN/'
#     for i in range(len(date_evento)):
#         plot_function(ST[i, :-3], depth[:-3], str(date_evento[i]), path+e+'/Crudo/', str(date_evento[i]))
#         plot_function(ST_anom[i, :-3], depth[:-3], u'Anomalías ST - '+str(date_evento[i]), path+e+'/Anomalies/', str(date_evento[i]), True)



"EVENTOS ESCOGIDOS CON RASI"

# import glob
# import os
# import shutil
# for i in Dlon_50_75_TT_ini:
#     os.mkdir('/home/yordan/YORDAN/UNAL/TESIS_MAESTRIA/4_expo_2017/perfiles/TT/DURACION/50_75/'+i, 0777)
...
def mld(temp, prof, fechas, DELT = 0.2):
    ...
    mld_pandas = pnd.DataFrame(data=MLD_mod, index=fechas)
    return mld_pandas
...
for ch in ['TT', 'PP', 'PN']:
    ...
    for cri in ['DURACION']:
        ...
        for perc in ['75_100']:
            ...
            for c, f in zip(FECHAS_INICIO, FECHAS_FIN):

                print ch, cri, perc
                date_evento = pd.date_range(dt.datetime.strptime(c[:10],'%Y-%m-%d')-relativedelta(days=3), dt.datetime.strptime(f[:10],'%Y-%m-%d')+relativedelta(days=3), freq='D')
                C           = np.where(fechas == pd.Timestamp(c[:10])-relativedelta(days=3))[0][0] #Comienzo
                F           = np.where(fechas == pd.Timestamp(f[:10])+relativedelta(days=3))[0][0] #Final

                ST      = data_PN[C:F+1]
                #ST_anom = anom_PN[C:F+1]

                for i, j in enumerate(date_evento):
                    print j
                    path = '/home/yordan/YORDAN/UNAL/TESIS_MAESTRIA/4_expo_2017/perfiles/'+ch+'/'+cri+'/'+perc+'/'+str(c)+'/'
                    plot_function(ST[i, :-3], depth[:-3], str(j) + ' - ' + ch + ' (' + cri + ' ' + perc + '%)', path, str(j))
                    #plot_function(ST_anom[i, :-3], depth[:-3], u'Anomalías ST - '+str(date_evento[i]), path+e+'/Anomalies/', str(date_evento[i]), True)
