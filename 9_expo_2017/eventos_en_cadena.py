...
fechas_cumplen = []
criterio = 'mean_75_100_dur_75_100'
...
fechas_cumplen = pd.DatetimeIndex(set(fechas_cumplen))

MESES        = ['Ene', 'Feb', 'Mar', 'Abr', 'May', 'Jun', 'Jul', 'Ago', 'Sep', 'Oct', 'Nov', 'Dic']
Height = [1000]
for H in Height: 
	os.mkdir(u'/home/yordan/YORDAN/UNAL/TESIS_MAESTRIA/9_expo_2017/EVENTOS_ENCADENADOS/'+str(H)+'hPa/'+criterio, 0777)
	"Datos geopotencial"
	...
	for D in fechas_cumplen:
		ANOM_EVENTO = np.array([25, len(lat), len(lon)])
		os.mkdir(u'/home/yordan/YORDAN/UNAL/TESIS_MAESTRIA/9_expo_2017/EVENTOS_ENCADENADOS/'+str(H)+'hPa/'+criterio+'/'+str(D), 0777)

		for i, j in enumerate(range(-12,13)):
			cuando   = D + relativedelta(hours=6*j)
			pos_when = np.where(dates_ERA == cuando)[0][0]
			mes      = cuando.month
			hora     = cuando.hour
			clima    = CICLO[MESES[mes-1]+'_'+str(hora)]
			ANOM_EVENTO[i,:,:] = Geo_pot[pos_when] - clima

		min_evnt = np.nanmin(ANOM_EVENTO)
		max_evnt = np.nanmax(ANOM_EVENTO)

		for i, j in enumerate(range(-12,13)):
			print str(H)+'hPa/'+criterio+'/'+str(D), i
			cuando = D + relativedelta(hours=6*j)
			title  = u'Anomalía altura geopotencial - '+str(cuando)+'\n'+criterio+' - '+str(H)+' hPa'
			path   = u'/home/yordan/YORDAN/UNAL/TESIS_MAESTRIA/9_expo_2017/EVENTOS_ENCADENADOS/'+str(H)+'hPa/'+criterio+'/'+str(D)+'/'+str(cuando)
			Pos    = '%02d' % (i,)
			plotear(lat[-1], lat[0], lon[0], lon[-1], 15, 15, lon, lat, ANOM_EVENTO[i], min_evnt, max_evnt, u'm', titulo, path, C_T='k', wind=False, mapa_u=None, mapa_v=None)
