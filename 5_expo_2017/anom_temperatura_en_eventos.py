...
path  = '/home/yordan/YORDAN/UNAL/TESIS_MAESTRIA/5_expo_2017/anom_temperatura_eventos/'
for ch in ['TT', 'PP', 'PN']:
                    ...
                    if cri1 != cri2:
                        ...
                        for Prof in [0, 7, 11]: #superficial, 9.8 metros de prof y 19.4 metros de prof
                            ...
                            for c, f, m in zip(FECHAS_INICIO, FECHAS_FIN, FECHAS_MAX):
                                ...
                                if   ch == 'TT': ST = anom_TT[C:F+1, Prof].T
                                elif ch == 'PP': ST = anom_PP[C:F+1, Prof].T
                                elif ch == 'PN': ST = anom_PN[C:F+1, Prof].T

                                vector_x = np.arange(-n_back_max, -n_back_max + len(date_evento))
                                ax1.plot(vector_x, ST)

                            ax1.axvline(x=0.0, color='k', alpha=0.7)
                            ax1.set_ylabel(u'Temperature Anomalies (°C)')
