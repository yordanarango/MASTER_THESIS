...
a = open('/home/yordan/UNAL/TESIS_MAESTRIA/17_expo_2018/ciclo_diurno_anual_presure_025_6h.bin', 'rb')
...
"Leyendo datos"
file    = nc.Dataset('/home/yordan/UNAL/TRABAJO_DE_GRADO/DATOS_Y_CODIGOS/DATOS/PRESION-SEA-LEVEL-ERA/MSLP_025x025_0x40N_120_55W.nc')
lat     = file.variables['latitude'][:]
lon     = file.variables['longitude'][:] - 360
tempo   = file.variables['time'][:]
CDFtime = utime('hours since 1900-01-01 00:00:0.0', calendar='gregorian')
DATES   = [CDFtime.num2date(x) for x in tempo]
DATES   = pd.DatetimeIndex(DATES) # Sólo va hasta 2016 08-31
...
for k in range(1, NM+1):
    ...
    pos_nc = [np.where(DATES == d)[0][0] for d in dates if d <= pd.Timestamp('2016-08-31 18:00:00')] #porque DATES sólo llega hasta esta fecha
    ...
    path.append('/home/yordan/UNAL/TESIS_MAESTRIA/24_expo_2018/PR_COMPOSITES/' + ch + '_CompPressure_JanDec_st'+str(S)+'_HMM'+str(NM)+'_anom')
